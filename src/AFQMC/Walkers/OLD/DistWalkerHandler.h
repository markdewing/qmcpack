...
class DistWalkerHandler: public WalkerHandlerBase 
{
  ...
  /// constructor
  DistWalkerHandler(afqmc::TaskGroup_& tg_, xmlNodePtr cur, AFQMCInfo& info, 
        RandomGenerator_t* r, ComplexMatrix& HF, int nbp): 
                WalkerHandlerBase(info,r),TG(tg_),
                ...
                popcontrol_on_master(true),HFMat(HF),nback_prop(nbp)
  {
    parse(cur);
    setup(); 
  }
...
// checking
  ...
  inline int GlobalPopulation() {
    ...
    if(head)
      res += tot_num_walkers;
    res = TG.Global().reduce_value(res);
    return res;
  }
  ...
  enum walker_data { INFO=0, SM=1, WEIGHT=2, ELOC=3, ELOC_OLD=4, OVLP_A=5, OVLP_B=6, OLD_OVLP_A=7, OLD_OVLP_B=8, PROPAGATORS=9, HEAD=10, TAIL=11, SMN=12, COS_FAC=13, WEIGHT_FAC=14 };

  // n is zero-based
  ComplexType* getSM(int n) { if(n>=maximum_num_walkers) {return NULL;} else {return &((walkers)[walker_size*n+data_displ[SM]]);} }
  // get the Slater Matrix for timestep "n" for back propagation (i.e., the step when we begin to store the propagators.)
  ComplexType* getSMN(int n) { if(n>=maximum_num_walkers) {return NULL;} else {return &((walkers)[walker_size*n+data_displ[SMN]]);} }
  ...
  ComplexType* getPM(int n) {
    if (n>=maximum_num_walkers) {
      return NULL;
    } else {
      ComplexType* B = &(walkers)[walker_size*n+data_displ[PROPAGATORS]];
      // head marks the location of the "current" propagator matrix.  By default head
      // should be set to zero. As more Propagator matrices are constructed the head index
      // increments along the array and loops around the boundary, if it is surpassed.
      int head = static_cast<int>(((walkers)[walker_size*n+data_displ[HEAD]]).real());
      B += head*disp_bp;
      return B;
    }
  }
  void incrementPM(int n) {
    if (n>=maximum_num_walkers) {
    } else {
      // Current value of HEAD.
      int head = static_cast<int>(((walkers)[walker_size*n+data_displ[HEAD]]).real());
      // Update HEAD and impose periodic boundary conditions.
      (walkers)[walker_size*n+data_displ[HEAD]] = (head+1)%nback_prop;
    }
  }
  void decrementPM(int n) {
    if (n>=maximum_num_walkers) {
    } else {
      // Current value of HEAD.
      int head = static_cast<int>(((walkers)[walker_size*n+data_displ[HEAD]]).real());
      // Update HEAD and impose periodic boundary conditions.
      (walkers)[walker_size*n+data_displ[HEAD]] = (head-1+nback_prop)%nback_prop;
    }
  }
  bool isPMBufferFull(int n) {
    if (n>=maximum_num_walkers) {
      return false;
    } else {
      // Current value of HEAD.
  ...
  ComplexType getCosineFactor(int n) {
    if (n>=maximum_num_walkers) {
      return 0;
    } else {
      int head = static_cast<int>(((walkers)[walker_size*n+data_displ[HEAD]]).real());
      return (walkers)[walker_size*n+data_displ[COS_FAC]+head];
    }
  }
  void setCosineFactor(int n, ComplexType cfac) {
    if (n>=maximum_num_walkers) {
      return;
    } else {
      int head = static_cast<int>(((walkers)[walker_size*n+data_displ[HEAD]]).real());
      (walkers)[walker_size*n+data_displ[COS_FAC]+head] = cfac;
    }
  }
  ComplexType getWeightFactor(int n) {
    if (n>=maximum_num_walkers) {
      return zero;
    } else {
      int head = static_cast<int>(((walkers)[walker_size*n+data_displ[HEAD]]).real());
      return (walkers)[walker_size*n+data_displ[WEIGHT_FAC]+head];
    }
  }
  void setWeightFactor(int n, ComplexType wfac) {
    if (n>=maximum_num_walkers) {
      return;
    } else {
      int head = static_cast<int>(((walkers)[walker_size*n+data_displ[HEAD]]).real());
      (walkers)[walker_size*n+data_displ[WEIGHT_FAC]+head] = wfac;
    }
  }
  int getNBackProp() { return nback_prop; }
  ...
  void copy_slater_matrix_to_historic_slater_matrix(int n) {
    ComplexType* current = getSM(n);
    ComplexType* historic = getSMN(n);
    std::copy(current, current+nrow*ncol, historic);
  }
  ...
  //   - old_overlap_alpha:    2
  //   - old_overlap_beta:     2
  //   - propagators:          NBACK_PROP*NCOL_BP*NROW_BP=DISP_BP
  //   - head:                 1
  //   - tail:                 1
  //   - SlaterMatrixN:        Same size as Slater Matrix
  //   - cos_fac :             NBACK_PROP
  //   - weight_fac:           NBACK_PROP
  //   Total: 16+2*ROW*NCOL+DISP_BP+2*NBACK_PROP
  int type, nrow, ncol, nrow_bp, ncol_bp, disp_bp, bp_size, nback_prop;
  int walker_size, data_displ[15], walker_memory_usage;
