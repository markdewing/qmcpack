...
void DistWalkerHandler::parse(xmlNodePtr cur)
{
    ...
    if(load_balance_alg.find("all")!=std::string::npos) {
      ...
      app_error()<<" Error: Unknown load balancing algorithm: " <<load_balance_alg <<" \n";
      APP_ABORT("");
    }  
...
void DistWalkerHandler::setup()
{
  ...
  //   Total: 16+2*NROW*NCOL+DISP_BP+2*NBACK_PROP
  int cnt=0;
...
// MAM: FIX FIX FIX: Should only be counted if nback_prop > 0 !!! Otherwise wasting lots of memory
  data_displ[9] = cnt; cnt+=bp_size; // propagators for back propagation.
  data_displ[10] = cnt; cnt+=1; // current location of propagator matrix in circular buffer.
  data_displ[11] = cnt; cnt+=1; // position of tail of circular buffer.
  data_displ[12] = cnt; cnt+=nrow*ncol; // position of tail of circular buffer.
  data_displ[13] = cnt; cnt+=nback_prop; // Cosine factors associated to weights along BP path.
  data_displ[14] = cnt; cnt+=nback_prop; // Missing imaginary weight factors along BP path.
  walker_size = cnt;
  walker_memory_usage = walker_size*sizeof(ComplexType);

  walkers.setup(head,std::string("DistWalkerHandler_")+std::to_string(TG.getTGNumber()),TG.TG_local().impl_);
  tot_num_walkers=maximum_num_walkers=0;

  int dummy=0;
#if defined(HAVE_MPI)
  MPI_Comm_size(TG.TG_heads().impl_,&nproc_heads);
  MPI_Comm_rank(TG.TG_heads().impl_,&rank_heads);
#else
    ...
    if(tot_num_walkers < n) {
      ...
      for(int i=0; i<maximum_num_walkers; i++) {
        if(walkers[walker_size*i+data_displ[INFO]].real() < 0) {   
          ...
          *(walkers.begin()+walker_size*i+data_displ[HEAD]) = ComplexType(0);
          *(walkers.begin()+walker_size*i+data_displ[TAIL]) = ComplexType(bp_size);
          std::copy(S0->begin(),S0->end(),walkers.begin()+walker_size*i+data_displ[SMN]);
          cnt++;
        }
        if(cnt == n) break;
      }
  ...
  if(load_balance_alg.find("all")!=std::string::npos) {
    ...
    if(head) {
      ...
      if(comm_rank==0) {
        ...
        MPI_Gatherv(buffer,nn,MPI_DOUBLE_COMPLEX,commBuff.data(),counts.data(), displ.data(),MPI_DOUBLE_COMPLEX,0,comm);
        ...
        MPI_Scatterv(commBuff.data(),counts.data(), displ.data(),MPI_DOUBLE_COMPLEX,buffer,nn,MPI_DOUBLE_COMPLEX,0,comm);
...
// trying to understand time spend idle
  ...
  if(pop_control_type.find("simple")!=std::string::npos) {
    ...
    if(head) {
      ...
      afqmc::BasicWalkerData(*this,curData,TG.TG_heads().impl_);
...
#ifdef HAVE_MPI_IBCAST
  MPI_Request req3;
  MPI_Ibcast(curData.data(),curData.size(),MPI_INT,0,TG.TG_local().impl_,&req3);
#else
  MPI_Bcast(curData.data(),curData.size(),MPI_INT,0,TG.TG_local().impl_);
#endif
