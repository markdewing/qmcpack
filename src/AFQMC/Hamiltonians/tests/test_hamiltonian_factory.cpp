...
";
    ...
    if(file_exists("./wfn.dat")) {
        ...
        boost::multi_array<ComplexType,1> hij = ham.halfRotatedHij(CLOSED,
        ...
        boost::multi_array<ComplexType,3> GM(extents[1][NMO][NMO]);
        boost::multi_array_ref<ComplexType,1> G0M(GM.origin(),extents[NMO*NMO]);
        Ovlp = SDet.MixedDensityMatrix(TrialWfn,OrbMat[0],
            GM[0],false);
        REQUIRE( real(Ovlp*Ovlp) == Approx(1.0) );
        REQUIRE( imag(Ovlp*Ovlp) == Approx(0.0) );

        boost::multi_array<ComplexType,1> X(extents[Spvn.shape()[1]]);
        using ma::T;
        ma::product(2.0*sqrtdt,T(Spvnview),
                    G0M[indices[range_t(0,NMO*NMO)]],0.,X);
        ComplexType Xsum=0;
        ...
        boost::multi_array<ComplexType,1> vHS(extents[NMO*NMO]);
...
";
    ...
    if(file_exists("./wfn.dat")) {
        ...
        //assert(gTG.getTotalNodes()>1);
        boost::multi_array<ComplexType,1> hij = ham.halfRotatedHij(COLLINEAR,
        ...
        boost::multi_array<ComplexType,3> GM(extents[2][NMO][NMO]);
        boost::multi_array_ref<ComplexType,1> G0M(GM.origin(),extents[2*NMO*NMO]);
