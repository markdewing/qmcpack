...
bool GeneralSingleDeterminant::getHamiltonian(HamPtr ham0)
{

  spinRestricted = true; 
  closed_shell = false;
  if(wfn_type==0 && NCA==NCB && NAEA == NAEB && init_type != "diagh1") {
  ...
  if(!readHamFromFile) {
    ...
    if(!ham0->createHamiltonianForPureDeterminant(dm_type,useFacHam,isOcc,isOcc,hij,SMSpHijkl,cutoff)) {
  ...
  if(init_type == "diagh1" && !readHamFromFile) {
    ...
    if(NCA==NCB && NAEA == NAEB && spinRestricted) {
      closed_shell = std::equal(OrbMat.begin(),OrbMat.begin()+NMO*NAEA,OrbMat.begin()+NMO*NAEA);
    }  
