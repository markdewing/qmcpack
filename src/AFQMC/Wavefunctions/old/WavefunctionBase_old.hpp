...
//        return true;
    ...
    void verifyWalkerData(WSetPtr wset, bool first , const int n=-1)
    {
      if(parallel)
    ...
    void serial_verifyWalkerData(WSetPtr wset, bool first, const int n=-1)
    {
      int nw = wset->numWalkers(true);
      if(nw==0) return;
    ...
    void serial_evaluateLocalEnergy(WSetPtr wset, bool first, const int n=-1)
    { 
      int nw = wset->numWalkers(true);
      if(nw==0) return;
      ComplexType ekin,epot,ovlp_a,ovlp_b;   
    ...
    virtual void dist_verifyWalkerData(WSetPtr wset, bool first, const int n=-1)
    {  
      serial_verifyWalkerData(wset,first,n);
    }  

    virtual void dist_evaluateLocalEnergy(WSetPtr wset, bool first, const int n=-1)
    {
      int nw = wset->numWalkers(true);
      if(nw==0) return;
      ComplexType ekin,epot,ovlp_a,ovlp_b;
    ...
    // no need to reimplement this in derived class
    void evaluateOverlap(WSetPtr wset, bool first, const int n=-1)
    {
      if(parallel) 
    ...
    virtual void dist_evaluateOverlap(WSetPtr wset, bool first, const int n=-1)
    {
      int nw = wset->numWalkers(true);
      if(nw==0) return;
    ...
    // no need to reimplement this in derived class
    void serial_evaluateOverlap(WSetPtr wset, bool first, const int n=-1)
    { 
      int nw = wset->numWalkers(true);
      if(nw==0) return;
      ComplexType ovlp_a,ovlp_b;
    ...
    virtual void evaluateOneBodyMixedDensityMatrix(WSetPtr wset, SPComplexSMVector* buf, int wlksz, int gfoffset, bool transposed, bool full=true) {
      APP_ABORT(" Error: evaluateOneBodyMixedDensityMatrix not implemented for this wave-function. \n\n\n");
    }

    virtual void evaluateOneBodyMixedDensityMatrix(const ComplexType* SlaterMat, ComplexMatrix& G, ComplexType& oa, ComplexType& ob)=0;
  ...
  protected:
    ...
    std::vector<ComplexType> ova_base,ovb_base,eloc_base;
