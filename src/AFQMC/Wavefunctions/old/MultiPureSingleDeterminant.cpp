...
bool MultiPureSingleDeterminant::getHamiltonian(HamPtr h)
{
  ...
  NuclearCoulombEnergy = static_cast<ValueType>(ham0->NuclearCoulombEnergy);
  ...
  if(runtype==-1) { // no local energy evaluation 
    ...
    if(!ham0->createHamiltonianForPureDeterminant(dm_type,useFacHam,all_orbs,all_orbs,hij[0],SMSpHijkl[0],cutoff)) {
    ...
    for(int ki=0; ki<ci.size(); ki++) {
      ...
      if(rotated_hamiltonian) {
        ...
        if(!ham0->createHamiltonianForGeneralDeterminant(dm_type,Am,haj[ki],SMSpHabkl[ki],cutoff)) {
        ...
        if(!ham0->createHamiltonianForPureDeterminant(closed?0:dm_type,useFacHam,isOcc_alpha,isOcc_beta,hij[ki],SMSpHijkl[ki],cutoff)) {
          app_error()<<"Error in createHamiltonianForPureDeterminant. \n";
          return false;
        }
        m1+=hij[ki].size();
...
void MultiPureSingleDeterminant::evaluateOneBodyMixedDensityMatrix(WSetPtr wset, SPComplexSMVector* buf, int wlksz, int gfoffset, bool transposed, bool full)
{
...
void MultiPureSingleDeterminant::evaluateOneBodyMixedDensityMatrix(const ComplexType* SlaterMat, SPComplexMatrix& G, ComplexType& oa, ComplexType& ob)
{
   //Unused, primarily for interface consistency.
  local_evaluateOneBodyMixedDensityMatrixFull(SlaterMat, oa, G, true);
  ob = ComplexType(1.0,0.0);
}
...
/*
      if(first_pass) {
        ...
        if(!ham0->createHamiltonianForPureDeterminant(dm_type,false,all_orbs,all_orbs,thij,H2,cutoff)) {
        ...
        if(!ham0->createHamiltonianForPureDeterminant(dm_type,true,all_orbs,all_orbs,thij,H1,cutoff)) {
          APP_ABORT(" Error during testing. \n\n\n"); 
        }
        ...
        if(!ham0->createHamiltonianForPureDeterminant(dm_type,false,all_orbs,all_orbs,tthij,H0,cutoff)) {
          APP_ABORT(" Error during testing. \n\n\n");
        }
      }
   ...
   if(rotated_hamiltonian) {  // since I do not have a way to do this right now  
    ...
    ke = ke_nume/deno;
      ...
      for(int ki=0; ki<ci.size(); ki++) {
        ...
        {
          ke += std::conj(ci[ki])*ci[ki]*ham0->H(*(it+i),*(it+i));
          for(int j=i+1; j<NAEA+NAEB; j++) 
            pe += std::conj(ci[ki])*ci[ki]*(ham0->H(*(it+i),*(it+j),*(it+i),*(it+j)) - ham0->H(*(it+i),*(it+j),*(it+j),*(it+i)));
        }
   ...
   return sucess;
    ...
    if(myComm->rank()==0) {
      ...
      for(int ki=0; ki<nci; ki++) {
        ...
        for(int i=0; i<NAEA+NAEB; i++)
        { 
          let += ham0->H(*(it+i),*(it+i));
          for(int j=i+1; j<NAEA+NAEB; j++) { 
            let += ham0->H(*(it+i),*(it+j),*(it+i),*(it+j)) - ham0->H(*(it+i),*(it+j),*(it+j),*(it+i));
          }
        }
        ...
        for(int kj=ki+1; kj<nci; kj++) {
          ...
          if(cnt==0) {
            ...
            let=ham0->H(n0,n1);
            for(int i=0; i<nterms; i++)
              let+=ham0->H(n0,occ[i],n1,occ[i]) - ham0->H(n0,occ[i],occ[i],n1);
            hm(ki,kj)=let*sg; 
          } else if(cnt==4) {
            hm(ki,kj) = sg*(ham0->H(n0,n1,n2,n3) - ham0->H(n0,n1,n3,n2)); 
          } else {
            hm(ki,kj) =  zero;
          }
          ...
          for(int ib=ia+1; ib<NAEA; ib++) {
            ...
            for(int ic=0; ic<NMO-NAEA; ic++) {
              ...
              for(int id=ic+1; id<NMO-NAEA; id++) {
                int d = vira[id]; 
                ValueType Hij = ham0->H(a,b,c,d) - ham0->H(a,b,d,c); 
