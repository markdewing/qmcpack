...
class PureSingleDeterminant: public WavefunctionBase
{
  ...
  public:
    ...
    setup_vn_occ_indx(true),rotated_hamiltonian(false),starting_det("")
    {}
    ...
    void evaluateOneBodyMixedDensityMatrix(const ComplexType* SlaterMat, ComplexMatrix& G, ComplexType& oa, ComplexType& ob);
    ...
    void dist_evaluateLocalEnergy(WSetPtr wset , bool first, const int n=-1 );

    void evaluateOverlap(const ComplexType* , ComplexType& ovl_alpha, ComplexType& ovl_beta, const int n=-1 );

    void dist_evaluateOverlap(WSetPtr wset, bool first, const int n=-1 );
    ...
    void evaluateOneBodyMixedDensityMatrix(WSetPtr wset, SPComplexSMVector* buf, int wlksz, int gfoffset, bool transposed, bool full=true);
    ...
    ComplexType* getOrbMat() { return OrbMat.data(); }
    int getOrbSize () { return 2*NMO; }
    std::vector<ComplexType> getCiCoeff() { return ci; }
  ...
  //private:
    ...
    bool getRotated(){ return rotated_hamiltonian; }
    ...
    ComplexMatrix OrbMat_for_walker_init; 
...
//    std::vector<IndexType> virtual_alpha; 
 //   std::vector<IndexType> virtual_beta; 
    ...
    // Dummy ci coefficients for compatability with multi determinant.
    std::vector<ComplexType> ci = {ComplexType(1.0)};
