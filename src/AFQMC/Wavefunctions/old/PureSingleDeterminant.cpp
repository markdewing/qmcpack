...
#include "AFQMC/Utilities/readWfn.hpp"
...
bool PureSingleDeterminant::setup_local()
{
  ...
  if(init_type == "ground" && !rotated_hamiltonian && !readHamFromFile ) {
    ...
    for (int i = 0; i<NAEA; i++) OrbMat[occup_alpha[i]][i] = 1;
    for (int i = 0; i<NAEB; i++) OrbMat[occup_beta[i]][i] = 1;
  } else {
  ...
  OrbMat_for_walker_init = OrbMat;
  if(starting_det != "")
  {
    readWfn(starting_det,OrbMat_for_walker_init,NMO,NAEA,NAEB);  
  }  
...
bool PureSingleDeterminant::getHamiltonian(HamPtr ham0)
{
  ...
  if(!readHamFromFile) {
    ...
    if(rotated_hamiltonian) {
      ...
      if(!ham0->createHamiltonianForGeneralDeterminant(dm_type,OrbMat,haj,SMSpHabkl,cutoff)) {
      ...
      if(!ham0->createHamiltonianForPureDeterminant(dm_type,useFacHam,isOcc_alpha,isOcc_beta,hij,SMSpHijkl,cutoff)) {
...
// testing
    myComm->barrier();
    app_log()<<" Before split Ham. " <<std::endl;
    std::cout.flush();
  ...
  app_log()<<std::endl <<"*********************************************************************: \n"
    ...
    if(rotated_hamiltonian) {
        std::copy(OrbMat_for_walker_init.data(),OrbMat_for_walker_init.data()+NAEA*NMO,HF.data());
        if(wfn_type==0 || initialDet==0)
            std::copy(OrbMat_for_walker_init.data(),OrbMat_for_walker_init.data()+NAEA*NMO,HF.data()+NAEA*NMO);
        else
            std::copy(OrbMat_for_walker_init.data()+NAEA*NMO,OrbMat_for_walker_init.data()+2*NAEA*NMO,HF.data()+NAEA*NMO);
    } else {
...
void PureSingleDeterminant::evaluateOneBodyMixedDensityMatrix(WSetPtr wset, SPComplexSMVector* buf, int wlksz, int gfoffset, bool transposed, bool full)
{
...
void PureSingleDeterminant::evaluateOneBodyMixedDensityMatrix(const ComplexType* SlaterMat, SPComplexMatrix& G, ComplexType& oa, ComplexType& ob)
{
  // For interface compatability with multi-determinant companion.
  SPComplexType* dm = G.data();
  local_evaluateOneBodyMixedDensityMatrix(SlaterMat, oa, ob, dm, true);
}
...
void PureSingleDeterminant::local_evaluateOneBodyTrialDensityMatrix(bool full)
{
  ...
  void PureSingleDeterminant::dist_evaluateOverlap(WSetPtr wset , bool first, const int n ) {
    // testing
  ...
  void PureSingleDeterminant::dist_evaluateLocalEnergy(WSetPtr wset , bool first, const int n ) {
    // testing
