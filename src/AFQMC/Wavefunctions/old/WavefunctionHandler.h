...
class WavefunctionHandler: public MPIObjectBase, public AFQMCInfo
{
  ...
  public:
    ...
    inline ComplexType* getOrbMat(const std::string& type) {
      if(type == std::string("ImportanceSampling")) {
        return ImpSampWfn->getOrbMat();
      } else if(type == std::string("Estimator")) {
        if(EstimatorWfn!=NULL) {
          return EstimatorWfn->getOrbMat();
        }
      } else {
        APP_ABORT("Unknown wavefunction type in getOrbMat(). \n");
      }
    }
    inline int getOrbSize(const std::string& type) {
      if(type == std::string("ImportanceSampling")) {
        return ImpSampWfn->getOrbSize();
      } else if(type == std::string("Estimator")) {
        if(EstimatorWfn!=NULL) {
          return EstimatorWfn->getOrbSize();
        }
      } else {
        APP_ABORT("Unknown wavefunction type in getOrbSize(). \n");
      }
    }
    inline std::vector<ComplexType> getCiCoeff(const std::string& type) {
      if(type == std::string("ImportanceSampling")) {
        return ImpSampWfn->getCiCoeff();
      } else if(type == std::string("Estimator")) {
        if(EstimatorWfn!=NULL) {
          return EstimatorWfn->getCiCoeff();
        }
      } else {
        APP_ABORT("Unknown wavefunction type in getCiCoeff(). \n");
      }
    }
    ...
    inline void evaluateLocalEnergyAndOverlap(const std::string& type, const int n, WSetPtr wset)
    {
        if(type == std::string("ImportanceSampling")) {
    ...
    inline void verifyWalkerData(const std::string& type, const int n, WSetPtr wset)
    {
        if(type == std::string("ImportanceSampling")) {
    ...
    inline void evaluateOverlap(const std::string& type, const int n, WSetPtr wset) 
    {
        if(type == std::string("ImportanceSampling")) {
    ...
    inline void evaluateOneBodyMixedDensityMatrix(const std::string& type, WSetPtr wset, SPComplexSMVector* buf, int wlksz, int gfoffset, bool transposed, bool full=true) 
    {
      if(type == std::string("ImportanceSampling")) {
    ...
    inline void evaluateOneBodyMixedDensityMatrix(const std::string& type, const ComplexType* SlaterMat, SPComplexMatrix& G)
    {
      if (type == std::string("ImportanceSampling")) {
        ImpSampWfn->evaluateOneBodyMixedDensityMatrix(SlaterMat, G);
      } else if (type == std::string("Estimator")) {
        EstimatorWfn->evaluateOneBodyMixedDensityMatrix(SlaterMat, G);
      } else {
        APP_ABORT("Unknown wavefunction type in evaluateOneBodyMixedDensityMatrix.\n");
      }
    }
