...
class SlaterDetOperations: public AFQMCInfo 
{
  public:

    SlaterDetOperations(AFQMCInfo& info):AFQMCInfo(info) //, ham(NULL) {}
    {
      GF.resize(2*NMO,NMO);
    ...
    void green_function(ComplexType* A, ComplexType* B, ComplexType& ovlp, SPComplexMatrix& G, bool getG=true, bool closed_shell=false) {
      ...
      if (closed_shell) {
        // Copy over green's function due to how wavefunctions are usually stored.
        std::copy(tGF.data(), tGF.data()+NMO*NMO, tGF.data()+NMO*NMO);
      } else {
        // SS0(beta) = B(beta) * S1
    ...
    // Multi-determinant A.
    void green_function(ComplexType* A, ComplexType* B, ComplexType& ovlp, SPComplexMatrix& G, std::vector<ComplexType> coeffs, int orbsize, bool closed_shell=false) {
      ComplexType denom = ComplexType(0.0);
      G = ComplexType(0.0);
      for (int i = 0; i < coeffs.size(); i++) {
        green_function(A+i*orbsize, B, ovlp, tGF, true, closed_shell);
        G = G + myconj(coeffs[i])*ovlp*tGF;
        denom += myconj(coeffs[i])*ovlp;
      }
      G *= (SPComplexType(1.0,0.0)/static_cast<SPComplexType>(denom));
    }
