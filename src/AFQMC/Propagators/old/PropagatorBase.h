...
class PropagatorBase: public MPIObjectBase, public AFQMCInfo
{
  ...
  virtual void Propagate(int steps, int& steps_total, WSetPtr, RealType& E1)=0;

  virtual bool parse(xmlNodePtr)=0;

  virtual bool setup(std::vector<int>&,SPComplexSMVector*,HamPtr,WfnPtr, RealType dt, hdf_archive&, const std::string&, MPI_Comm, MPI_Comm, MPI_Comm)=0;
  ...
  virtual void benchmark(std::string&,int,int,int,WSetPtr)=0;
