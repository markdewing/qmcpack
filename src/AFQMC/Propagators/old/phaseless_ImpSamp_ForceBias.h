...
class phaseless_ImpSamp_ForceBias: public PropagatorBase
{
  ...
  phaseless_ImpSamp_ForceBias(Communicate *c,  RandomGenerator_t* r) : PropagatorBase(c,r), substractMF(true),use_eig(false),first(true),max_weight(100),apply_constrain(true),save_memory(false),vbias_bound(3.0),imp_sampl(true),hybrid_method(false),eloc_from_Spvn(false),sizeOfG(0),walkerBlock(0),test_cnter(0),cutoff(1e-6),fix_bias(0),n_reading_cores(-1),intgs_per_block(2000000),nback_prop_steps(0),sz_pin_field_mag(0.0),sz_pin_field_file("")
  {
  } 

  ~phaseless_ImpSamp_ForceBias() {}

  void Propagate(int steps, int& steps_total, WSetPtr, RealType& E1);

  bool setup(std::vector<int>&,SPComplexSMVector*,HamPtr,WfnPtr,RealType, hdf_archive&, const std::string&,MPI_Comm,MPI_Comm,MPI_Comm);
  ...
  void benchmark(std::string&,int,int,int,WSetPtr);

  SPValueSMVector* getDvn() { return &Dvn; }

  SPValueSMSpMat* getSpvn() { return &Spvn; }

  int getNBackProp() { return nback_prop_steps; }
  ...
  double sz_pin_field_mag;
  std::string sz_pin_field_file;

  // Number of propagator matrices ~ amount of imaginary time we back propagate by.
  ...
  // Temporary storage for constructing propagator matrix for back propagation.
  ComplexMatrix TB1;
  ComplexMatrix TB2;
  ComplexMatrix vHS_BT;
  ...
  void serial_propagation_single_step(WSetPtr, RealType& E1);

  void dist_propagation_single_step(WSetPtr, RealType& E1);
  void dist_propagation_single_step_construct_propagator(WSetPtr, RealType& E1);

  // in this case, vbias is calculated for step 0 and fixed for all subsequent sub steps
  void serial_propagation_multiple_steps(int steps, WSetPtr, RealType& E1);

  void dist_propagation_multiple_steps(int steps, WSetPtr, RealType& E1);
  ...
  void construct_propagator_matrix(ComplexType* PM, ComplexType& factor, int order=-1);

  void addvHS(SPComplexSMVector *buff, int nw, int sz, WSetPtr wset); 

  void addvHS_multiple(int n, SPComplexSMVector *buff, int nw, int sz, WSetPtr wset); 
