...
bool phaseless_ImpSamp_ForceBias::setup(std::vector<int>& TGdata, SPComplexSMVector* v, HamPtr ham,WfnPtr w, RealType dt_, hdf_archive& dump_read, const std::string& hdf_restart_tag, MPI_Comm tg_comm, MPI_Comm node_comm, MPI_Comm node_heads_comm)
{
  ...
  ComplexMatrix Hadd;
  if(sz_pin_field_file != "") { 
    Hadd.resize(2*NMO,NMO);
    for(int i=0; i<2*NMO; i++) 
     for(int j=0; j<NMO; j++) 
      Hadd(i,j)=0;
    // read h0 from file, scale alpha by -sz_pin_field and beta by sz_pin_field  
    readMat(sz_pin_field_file,Hadd);
    for(int i=0; i<NMO; i++)
      for(int j=0; j<NMO; j++) {
        Hadd(i,j) *= -sz_pin_field_mag;
        Hadd(i+NMO,j) = -Hadd(i,j); 
      }  
    for(int i=0; i<NMO; i++)
      for(int j=i+1; j<NMO; j++) {
        if( std::abs(Hadd(i,j) - myconj(Hadd(j,i))) > 1e-8  ) {
          app_error()<<" Error: Found non-Hermitian sz pinning hamiltonian. " <<std::endl;
          app_error()<<i <<" " <<j <<" " <<Hadd(i,j) <<" " <<Hadd(j,i) <<std::endl;  
          APP_ABORT(" Error: Found non-Hermitian sz pinning hamiltonian. \n");  
        }
      }
    for(int i=0; i<NMO; i++)
      for(int j=i+1; j<NMO; j++) {
...
//  out_debug.open(str_.c_str());
  ...
  if(!read_Spvn_from_file && !parallel_factorization) {
    ...
    if(rank()==0) {
      ...
      if(use_eig) {
        ...
        APP_ABORT(" calculateHSPotentials_Diagonalization disabled. \n");  
  ...
  Propg_H1_dense.resize(Hadd.size1(), NMO);
  if(rank()==0) { 
    ...
    if(substractMF) {
      ...
      for(int i=0; i<NMO; i++)
        for(int j=0; j<NMO; j++)
          Hadd(i,j) += static_cast<ComplexType>(SPvHS(i,j));
      if(Hadd.size1() == 2*NMO)
        for(int i=0; i<NMO; i++)
          for(int j=0; j<NMO; j++)
            Hadd(i+NMO,j) += static_cast<ComplexType>(SPvHS(i,j));
    ...
    if(Hadd.size1() == NMO) {
      ...
      for(int i=0; i<NMO; i++) 
        for(int j=0; j<NMO; j++) { 
          Hadd(i,j) += vn0(i,j);
          Hadd(i+NMO,j) += vn0(i,j);
        }
    }

    Timer.reset("Generic1");
    Timer.start("Generic1");
    ham->calculateOneBodyPropagator(cutoff, dt, Hadd, Propg_H1, Propg_H1_dense);
    Timer.stop("Generic1");
    ...
    myComm->bcast<char>(reinterpret_cast<char*>(Propg_H1_dense.data()), sizeof(ComplexType)*Propg_H1_dense.size());
...
void phaseless_ImpSamp_ForceBias::Propagate(int steps, int& steps_total, WSetPtr wlk, RealType& E1)
{
  ...
  if(fix_bias > 0) {
    ...
    if(parallelPropagation) {
      ...
      if (nback_prop_steps > 0) {
        for (int i = 0; i < steps; i++, steps_total++) {
          dist_propagation_single_step_construct_propagator(wlk,E1);
        }
      } else {
        for (int i = 0; i < steps; i++, steps_total++) {
...
void phaseless_ImpSamp_ForceBias::serial_propagation_single_step(WSetPtr wset, RealType& Eshift)
{
  ...
  if(hybrid_method) { 
    if(imp_sampl)
      wfn->evaluateOverlap(wset);
  } else {
      wfn->evaluateLocalEnergy(wset);
  }
...
void phaseless_ImpSamp_ForceBias::dist_propagation_single_step(WSetPtr wset, RealType& Eshift)
{
...
void phaseless_ImpSamp_ForceBias::dist_propagation_single_step_construct_propagator(WSetPtr wset, RealType& Eshift)
{
  // structure in TG [hybrid_w, MFfactor, G(1:{2*}NMO*NMO), sigma(1:nCholVecs), vHS(1:{2*}NMO*NMO)] 
  //  1. You either need to send sigma or communicate back vbias, choosing the former 
  //     In principle, you can replace sigma by vbias on the return communication if you need to accumulate vbias
  ...
  Timer.start("Propagate::propg");
    for(int i=0; i<nw; i++) {
      ...
      // Construct full propagator matrix (PM)
      ComplexType* PM;
      PM = wset->getPM(i);
      construct_propagator_matrix(PM, factor, 6);
      int disp = ((spinRestricted)?0:NMO*NMO);
      // Propagate Slater determinant
      DenseMatrixOperators::product(NMO, NAEA, NMO, one, PM, NMO, Sdet, NAEA, zero, S1.data(), NAEA);
      DenseMatrixOperators::product(NMO, NAEB, NMO, one, PM+disp, NMO, Sdet+NAEA*NMO, NAEB, zero, S2.data(), NAEB);
  ...
  for(int i=0; i<nw; i++) {
    ...
    wset->setCosineFactor(i, scale);
    wset->setWeightFactor(i, std::exp(-dt*(0.5*(ComplexType(0.0,eloc.imag())+ComplexType(0.0,oldeloc.imag())))));
    w0 *= ComplexType(scale*std::exp( -dt*( 0.5*( eloc.real() + oldeloc.real() ) - Eshift )),0.0);

    wset->setWalker(i,w0,eloc);
    // increment buffer for next propagator matrix.
    wset->incrementPM(i);
...
void phaseless_ImpSamp_ForceBias::dist_propagation_multiple_steps(int n, WSetPtr wset, RealType& Eshift)
{
...
void phaseless_ImpSamp_ForceBias::serial_propagation_multiple_steps(int n, WSetPtr wset, RealType& Eshift) {
}
...
void phaseless_ImpSamp_ForceBias::addvHS_multiple(int nstep, SPComplexSMVector *buff, int nw, int sz, WSetPtr wset)
{
...
// local_buffer is used as temporary storage for vbias in the same format as the TG buffer (transposed or not)
// if walkerBlock > 1, the MM algorithm is used and vbias and CV0 are resized. vbias is used as temporaty storage for both vbias and vHS for all walkers
void phaseless_ImpSamp_ForceBias::addvHS(SPComplexSMVector *buff, int nw, int sz, WSetPtr wset)
{
...
void phaseless_ImpSamp_ForceBias::construct_propagator_matrix(ComplexType* PM, ComplexType& factor, int order)
{
  ...
  // calculate exp(vHS)*Propg_H1_dense through a Taylor expansion of exp(vHS)
  Timer.start("Propagate::construct_propagator_matrix");
  TB1 = Propg_H1_dense;
  vHS_BT = Propg_H1_dense;
  for (int n=1; n<=order; n++) {
    ComplexType fact = ComplexType(0.0,1.0)*static_cast<ComplexType>(1.0/static_cast<double>(n));
    DenseMatrixOperators::product(NMO, NMO, NMO, fact, vHS.data(), NMO, TB1.data(),
                                  NMO, zero, TB2.data(), NMO);
    TB1  = TB2;
    vHS_BT += TB1;
  }
  // Construct full propagator = BT * exp(vHS) * BT = Propg_H1_dense * VBT
  DenseMatrixOperators::product(NMO, NMO, NMO, one, Propg_H1_dense.data(),
                                NMO, vHS_BT.data(), NMO, zero, PM, NMO);

  // Update spin down sectors.
  int disp = ((spinRestricted)?0:NMO*NMO);
  TB1 = Propg_H1_dense;
  vHS_BT = Propg_H1_dense;
  for(int n=1; n<=order; n++) {
    ComplexType fact = ComplexType(0.0,1.0)*static_cast<ComplexType>(1.0/static_cast<double>(n));
    DenseMatrixOperators::product(NMO, NMO, NMO, fact, vHS.data()+disp, NMO,
                                  TB1.data()+disp, NMO, zero, TB2.data()+disp, NMO);
    TB1  = TB2;
    vHS_BT += TB1;
  }
  DenseMatrixOperators::product(NMO, NMO, NMO, one, Propg_H1_dense.data()+disp, NMO,
                                vHS_BT.data()+disp, NMO, zero, PM+disp, NMO);
  Timer.stop("Propagate::construct_propagator_matrix");
}
...
void phaseless_ImpSamp_ForceBias::benchmark(std::string& blist, int maxnW, int delnW, int nrepeat, WSetPtr wlk)
{
