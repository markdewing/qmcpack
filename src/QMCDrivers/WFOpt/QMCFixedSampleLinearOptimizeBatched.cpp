...
QMCFixedSampleLinearOptimizeBatched::QMCFixedSampleLinearOptimizeBatched(MCWalkerConfiguration& w,
                                                           ...
                                                           SampleStack& samples,
                                                           Communicate* comm)
    : QMCLinearOptimizeBatched(w, psi, h, hpool, ppool, std::move(qmcdriver_input), std::move(vmc_input), population, samples, comm),
#ifdef HAVE_LMY_ENGINE
...
bool QMCFixedSampleLinearOptimizeBatched::processOptXML(xmlNodePtr opt_xml, const std::string& vmcMove, bool reportH5, bool useGPU)
{
  ...
  if (useGPU)
    ...
    VMCDriverInput vmc_input_copy = vmc_input_;
    vmcEngine = new VMCBatched(std::move(qmcdriver_input_copy),
                               std::move(vmc_input_copy),
                               population_,
                               Psi,
                               H,
                               psiPool,
