...
#include "Optimize/dumpDerivOptimization.h"
...
bool QMCOptimizeBatched::put(xmlNodePtr q)
{
  ...
  if (vmcEngine == 0)
  {
    ...
    { 
      ...
      vmcEngine = new VMCBatched(std::move(qmcdriver_input_copy), std::move(vmc_input_copy), population_, Psi, H, psiPool, samples_, myComm);
    }
  ...
  if (optSolver == 0)
  {
    ...
    {
      app_log() << "Test optimization (batched) to dump parameter derivatives: " << std::endl;
      optSolver = new dumpDerivOptimization<RealType>;
    }
    else
    {
